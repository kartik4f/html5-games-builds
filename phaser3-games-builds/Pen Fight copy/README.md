...
- **Power-Ups** — one of three pickups appears on the table now and then
  ...
  harder — same charged-glow-until-fired pattern), or an Ink Puddle
...
- **Computer opponents** — Easy/Medium/Hard AI with its own aim, reaction
  time, and power tuned per difficulty (see `js/AIController.js`). Vs
  Computer also gives you a brief "get ready" pause before the very
  first turn/round, and the AI's own pen can't be grabbed and fired by
  mistake — only your pen responds to your input.
