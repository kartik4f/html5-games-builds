...
four: a yellow **Speed Boost** (⚡) that supercharges whoever's pen
...
short window after it's taken; a blue **Shield** (🛡️) that saves its pen
from the very next elimination — pulled back onto the table with its
velocity zeroed instead of actually going out — until it's used or
expires; or a dark **Ink Puddle** that slows down any pen currently
sitting in it until it fades away on its own. A floating callout marks
the pickup, and a charged/active pen keeps a pulsing ⚡/🚁/🛡️ glow so
it's always clear who's carrying what. Pickup detection checks the pen's
...
everything else, and broadcasts each player's charged-but-not-yet-fired
state plus, per pen, whether it's *currently* mid-spin (Helicopter Shot)
or shielded, so all of that shows up for every connected client, not just
the player who picked it up.

**Fair turn order.** Who starts each match rotates round-robin (player 0,
then 1, then 2, …) instead of always being the same seat — persists across
restarts within the room (`GameRoom.mjs`'s `matchesPlayed`), so a Best of
3 series doesn't let one player keep the first-hit advantage the whole way
through. Chaos rooms have no turn order to rotate.

**Flick style** (room toggle, set once at Create Room — every client in
the room uses the same style). **Pull Back** is the original slingshot
feel: drag away from the pen, release to fling it the *opposite* way.
**Swipe** flips that: flick across the pen and it flies the *same* way you
swiped. The server and every client agree on which convention is active
via the room's `flickMode` config, so it's consistent regardless of who's
shooting.
