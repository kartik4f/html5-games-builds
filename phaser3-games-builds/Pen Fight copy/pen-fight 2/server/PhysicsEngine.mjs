...
export class PenBody {
  constructor(id, x, y) {
    ...
    // Shield power-up (see GameRoom.mjs's _checkEliminations) — saves
    // this pen from its next elimination while active, then it's
    // consumed. shieldMs auto-expires it if never used.
  ...
  // Shield — mirrors Pen.js's activateShield()/pullBackOntoTable() on
  // the client. Pulls the pen back to a valid spot on the table rather
  // than letting it go out, used by GameRoom.mjs's _checkEliminations
  // in place of a normal elimination when this pen is shielded.
  pullBackOntoTable(table) {
    const margin = Math.max(PEN.LENGTH / 2, PEN.WIDTH / 2) + 12;

    this.x = Math.max(table.X + margin, Math.min(table.X + table.WIDTH - margin, this.x));
    this.y = Math.max(table.Y + margin, Math.min(table.Y + table.HEIGHT - margin, this.y));
...
function integrate(pens, dt) {
  ...
  for (const p of pens) {
    ...
    if (p.shieldMs > 0) {
      p.shieldMs = Math.max(0, p.shieldMs - dt * 1000);

      if (p.shieldMs === 0) p.hasShield = false;
    }
