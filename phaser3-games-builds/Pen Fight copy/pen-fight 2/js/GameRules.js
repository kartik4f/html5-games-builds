...
import { MATCH, GAME_MODE, AI } from './GameConfig.js';

// Module-level, not per-instance — persists across GameScene.restart()
// within the same page load (a fresh GameRules is constructed every
// restart), so the starting player rotates match to match instead of
// always being player 0. Resets naturally on a full page reload.
...
export default class GameRules {
  constructor(scene, penManager) {
    ...
    // Vs Computer only — a brief pause before the very first turn (or,
    // in chaos mode, the free-for-all) begins, so the human player has
    // a moment to see the table and get oriented before the turn timer
    // starts counting down or an AI takes its first shot. Classroom
    // has no AI opponents (AI.PLAYER_IDS is empty there), so this
    // never applies to it.
    this.startGraceMs = AI.PLAYER_IDS.length > 0 ? AI.START_GRACE_MS : 0;

    // Rotating turn order — round-robins which player starts across
    // matches within this session, instead of always player 0 (a real
    // advantage in a game about landing the first hit). Modulo'd
    // against the *current* player count so it stays valid even if the
    // player switches between a 2-player and 4-player match.
    this.rotationStartIndex = localMatchCount % penManager.getPens().length;
    localMatchCount += 1;
  }
  ...
  start() {
    ...
    this.currentTurnIndex = this.rotationStartIndex;
    ...
    if (this.startGraceMs > 0) {
      // Lets GameScene show a "get ready" message during the pause —
      // canPlayerInteract() already returns false the whole time since
      // turnActive stays false until beginMatch() actually runs, so
      // neither the human nor the AI can act yet either way.
      this.scene.events.emit('match-grace', this.startGraceMs);

      this.scene.time.delayedCall(this.startGraceMs, beginMatch);
    } else {
      beginMatch();
    }
  }
  ...
  checkOutOfBounds() {
    ...
    for (const pen of this.penManager.getPens()) {
      ...
      if (pen.isCompletelyOutside(TABLE)) {
        // Shield pickup (see PowerUps.js) — saves this pen from going
        // out instead of eliminating it, once, then it's consumed.
        if (pen.hasShield) {
          pen.consumeShieldSave(TABLE);
          this.scene.events.emit('shield-saved', pen);
          continue;
        }
  ...
  eliminate(pen) {
    ...
    // Fired before any visual removal starts, so listeners can still
    // read the pen's current position/color for elimination juice.
    this.scene.events.emit('pen-eliminated', pen);
