...
const INSTANT_PICKUP_TYPES = [POWERUP_TYPE.SPEED, POWERUP_TYPE.HELICOPTER, POWERUP_TYPE.SHIELD];
...
export default class PowerUpManager {
  ...
  spawn() {
    ...
    const type =
      roll < 0.25
        ? POWERUP_TYPE.SPEED
        : roll < 0.5
          ? POWERUP_TYPE.HELICOPTER
          : roll < 0.75
            ? POWERUP_TYPE.SHIELD
            : POWERUP_TYPE.INK;
    const radius = type === POWERUP_TYPE.INK ? POWERUPS.INK_RADIUS : POWERUPS.PICKUP_RADIUS;
  ...
  createGraphics(type, x, y, radius) {
    ...
    if (type === POWERUP_TYPE.SPEED) {
      ...
      container.add(label);
    } else if (type === POWERUP_TYPE.HELICOPTER) {
      ...
      const label = this.scene.add.text(0, 0, '🛡️', { fontSize: '22px' }).setOrigin(0.5);
  ...
  updateActive(deltaMs) {
    ...
    if (INSTANT_PICKUP_TYPES.includes(type)) {
      for (const pen of this.penManager.getPens()) {
        // Capsule-vs-circle overlap, not just a center-to-center
        // distance check — see Pen.overlapsCircle()'s comment for why
        // that matters for a 145px-long pen against a ~26px pickup.
        if (pen.overlapsCircle(x, y, radius)) {
          if (type === POWERUP_TYPE.SPEED) {
            this.applySpeedBoost(pen);
          } else if (type === POWERUP_TYPE.HELICOPTER) {
            this.applyHelicopter(pen);
          } else {
            this.applyShield(pen);
          }
  ...
  applyHelicopter(pen) {
    pen.nextShotHelicopter = true;

    // Keeps the pen visibly "charged" (see Pen.js) until that shot is
    // actually taken (InputController.js activates it there).
    pen.setHelicopterCharged(true);

    audioFX.playPickup();
    burst(this.scene, pen.container.x, pen.container.y, 0x26c6da, { count: 16, speed: 200 });
    floatingText(this.scene, pen.container.x, pen.container.y - 40, '🚁 HELICOPTER SHOT!', '#00838f');

    this.scene.events.emit('powerup-collected', { type: POWERUP_TYPE.HELICOPTER, pen });
  }
  ...
  applyShield(pen) {
    ...
    audioFX.playPickup();
    burst(this.scene, pen.container.x, pen.container.y, 0x42a5f5, { count: 16, speed: 200 });
    floatingText(this.scene, pen.container.x, pen.container.y - 40, '🛡️ SHIELD!', '#1565c0');

    this.scene.events.emit('powerup-collected', { type: POWERUP_TYPE.SHIELD, pen });
  }
