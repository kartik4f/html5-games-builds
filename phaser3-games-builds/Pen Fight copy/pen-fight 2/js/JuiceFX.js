...
export class Trail {
  constructor(scene, color) {
    ...
    this.scene = scene;
    this._createEmitter(color);
  }

  _createEmitter(color) {
    this.emitter = this.scene.add.particles(0, 0, DOT_TEXTURE, {
      lifespan: 260,
      speed: 0,
  ...
  // Only needed if a pen's cosmetic color can change after its trail
  // already exists (see Pen.setStyle()/NetPen.setStyle()) — the tint
  // is baked into the emitter's startup config with no documented live
  // setter, so this just swaps in a freshly-made emitter with the new
  // color instead.
  setColor(color) {
