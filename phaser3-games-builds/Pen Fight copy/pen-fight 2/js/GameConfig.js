...
//--------------------------------------------------
// Controls — how a drag on a pen turns into a shot. Set from the
// Classroom/Vs Computer setup screens (local) or a room's Create Room
// screen (online, carried in the room's config so every client agrees
// on the same convention); read by InputController.js (local) and
// GameRoom.mjs/NetGameScene.js (online).
//--------------------------------------------------

export const CONTROLS = {
  // 'pullback' — classic slingshot feel: drag the pen backward, it
  // flies the *opposite* way on release (today's original behavior).
  // 'swipe'    — flick feel: swipe across the pen, it flies the *same*
  // way you swiped, like flicking a coin.
  FLICK_MODE: 'pullback',
};
...
export const POWERUPS = {
  ...
  // Shield pickup — saves this pen from its very next elimination
  // (instead of going out, it's pulled back onto the table with its
  // velocity zeroed), then it's consumed. ms it lasts before expiring
  // unused, same one-active-effect-at-a-time lifecycle as the others.
