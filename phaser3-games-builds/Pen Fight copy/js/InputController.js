...
export default class InputController {
  ...
  findPenForPointer(pointer, tolerance) {
    ...
    if (this.gameRules.isChaos) {
      pen = this.penManager.getPenAt(
        pointer.worldX,
        pointer.worldY,
        tolerance,
      );
    } else {
      pen = this.gameRules.getCurrentPen();
    }

    // Vs Computer — the human should never be able to grab and fire an
    // AI-controlled pen, in either turn-based or chaos mode.
    if (pen && AI.PLAYER_IDS.includes(pen.playerId)) return null;
  ...
  fire(pointerId, drag) {
    ...
    // A Speed Boost pickup (see PowerUps.js) multiplies exactly one
    // shot, then resets — 1 when no boost is active.
    const boost = drag.pen.nextShotBoost || 1;

    drag.pen.nextShotBoost = 1;

    // Clears the pen's "charged" glow the moment the boosted shot is
    // actually taken (a no-op if there was no boost to begin with).
    drag.pen.setBoosted(false);

    // A Helicopter Shot pickup (see PowerUps.js) charges exactly one
    // shot — captured here, then consumed/activated after shoot().
    const helicopterCharged = !!drag.pen.nextShotHelicopter;

    drag.pen.nextShotHelicopter = false;

    // Final impulse
    const impulse = {
      x: dx * INPUT.IMPULSE_MULTIPLIER * power * mass * boost,

      y: dy * INPUT.IMPULSE_MULTIPLIER * power * mass * boost,
    };

    drag.pen.shoot(drag.localPoint, impulse);

    if (helicopterCharged) drag.pen.activateHelicopter();
