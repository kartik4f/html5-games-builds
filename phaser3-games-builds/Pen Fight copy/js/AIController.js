...
export default class AIController {
  ...
  _takeShot(pen) {
    ...
    // Power-ups (see PowerUps.js) apply to the AI's shots too, exactly
    // like InputController.fire() — otherwise an AI pen that picks one
    // up just keeps glowing "charged" forever and never actually uses it.
    const boost = pen.nextShotBoost || 1;

    pen.nextShotBoost = 1;
    pen.setBoosted(false);

    const helicopterCharged = !!pen.nextShotHelicopter;

    pen.nextShotHelicopter = false;

    const impulse = {
      x: shotDx * INPUT.IMPULSE_MULTIPLIER * power * mass * boost,
      y: shotDy * INPUT.IMPULSE_MULTIPLIER * power * mass * boost,
    };
    ...
    if (helicopterCharged) pen.activateHelicopter();
