...
their setup screens). Every so often a pickup appears on the table: a
yellow **Speed Boost** (⚡) that supercharges whoever's pen touches it for
their very next shot — a floating "SPEED BOOST!" callout marks the pickup,
and the boosted pen keeps a pulsing ⚡ glow until that shot is actually
taken, so it's always clear who's charged up — or a dark **Ink Puddle**
that slows down any pen currently sitting in it until it fades away on its
own. In online rooms the server decides spawns/pickups authoritatively,
same as everything else (including broadcasting whose next shot is
boosted, so the glow shows up for every connected client, not just the
