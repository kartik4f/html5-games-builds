...
export default class GameRoom {
  constructor(code, config, onEmpty) {
    ...
    this.powerUp = null; // { id, kind: 'speed'|'ink', x, y, radius, ageMs }
  ...
  _trySpawnPowerUp(attempts = 20) {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      const kind = Math.random() < 0.5 ? 'speed' : 'ink';
      const radius = kind === 'speed' ? POWERUPS.PICKUP_RADIUS : POWERUPS.INK_RADIUS;
